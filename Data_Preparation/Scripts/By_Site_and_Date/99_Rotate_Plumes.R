...
temp <- timeDist%>%
  filter(PARVAL >= 5)%>%
  group_by(GLOBAL_ID,LOGDATE)%>%
  mutate(N = n())%>%
  ungroup()%>%
  select(GLOBAL_ID,LOGDATE,N)%>%
  distinct()
site <- "T0611100502"
date <- lubridate::ymd("2003-02-11")
  ...
  tryCatch({
    ...
    for (date in unique(as.list(originDates))) {
      ...
      if(nrow(mws)>2){
        ...
        mld(mws,coords = c("LONGITUDE","LATITUDE"),start_x = origin$LONGITUDE[1],
            start_y = origin$LATITUDE[1], date = str_replace_all(as.character(date),"-","_"),crs = 3310,
            weight = mws$PARVAL,out_folder = here("Data_Preparation/Analysis_Data/meanDirection"))
        
        # Import mean direction line
        mdLine <- st_read(here("Data_Preparation/Analysis_Data/meanDirection/meanDirection.gpkg"),
                          layer = paste0(site,"_",str_replace_all(date,"-","_")))
        # Define bearing
        bearing <- mdLine$Bearing
        ...
        # Calculate rotated convex hull > mcl
