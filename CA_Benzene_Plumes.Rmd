...
## Wilcox Test
`r format(nrow(exceedance), big.mark = ",")` of `r format(nrow(bd),big.mark = ",")` cleanup sites had an exceedance of 5 ug/L at the testing
boundary (`r format(100*(nrow(exceedance)/nrow(bd)),nsmall = 1)`). Figure 4 illustrates that when plumes are completely captured (spatially), they are generally smaller than if plumes extend to the boundary of a monitoring network. The results of the Wilcox test (Table 1) show a significant difference between the measured plume areas at sites where boundary exceedance was observed and at sites where entire plume capture was observed (figure 5). Plume areas are statistically larger at sites where plumes were entirely captured, and the majority of plumes exhibited boundary exceedance across their monitoring well extents. The median plume area where plume capture was observed was `r format(median(capture$Area_m),digits=0,nsmall=0,big.mark=",")` m^2^. The median plume area where boundary exceedance was observed was `r format(median(exceedance$Area_m),digits=0,nsmall=0,big.mark=",")` m^2^, more than twice as much area as plumes that were spatially captured.



```{r plumeAreaCompare, message = FALSE, warning=FALSE,fig.height = 2.5,fig.width=5, fig.cap="Visulization of plume areas by whether they exhibited boundary capture (green/left) or boundary exceedance (brown/right)."}


ggplot(areaCompare)+
  geom_boxplot(aes(x = BoundIntrsct, y = Area_m, fill = BoundIntrsct),outlier.alpha = .5)+
  scale_fill_manual(values = c("#1b9e77","#d95f02"))+
  coord_cartesian(ylim=c(0, 6000))+
  labs(x = "Boundary Exceedance", y = "Plume Area [m^2]",
       fill = "Plume at Boundary")
```
```{r wilcox, message = FALSE, warning=FALSE, fig.cap= "Results for Wilcox test on whether plumes that exhibited boundary exceedance are statistically different than plumes that did not exhibit boundary exceedance."}
wx <- areaCompare%>%
      rstatix::wilcox_test(Area_m ~ BoundIntrsct, p.adjust.method = "bonferroni")%>%
      as.data.frame()

pw.effect <- areaCompare%>%
      rstatix::wilcox_effsize(Area_m ~ BoundIntrsct)%>%
      as.data.frame()%>%
      select(effsize,magnitude)

tbl <- cbind(wx,pw.effect)%>%
  select(".y.",group1,group2,n1,n2,statistic,p,effsize)%>%
  mutate(effsize = round(effsize,4))

colnames(tbl) <- c("Y","Group 1", "Group 2","n1","n2","Statistic","p","Effect")

kbl(tbl,caption = "Wilcox Test Results")%>%
